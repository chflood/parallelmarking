...
#else
        fprintf(stderr, "GC queue overflow\n");
        abort();
#endif
    }
    // Copy pc/data items
...
#else
    jl_gc_ws_array_t *array2 = &mark_queue2->array;
#endif
...
JL_EXTENSION NOINLINE void gc_mark_loop(jl_ptls_t ptls)
{
    ...
    pop : {
        ...
        if (GC_MARK_L_marked_obj <= (uintptr_t)pc && (uintptr_t)pc < _GC_MARK_L_MAX)
            gc_mark_jmp(pc);
...
void jl_init_thread_heap(jl_ptls_t ptls)
{
    ...
    array->pc_start = (void **)malloc_s(JL_GC_QUEUE_INIT_SIZE * sizeof(void *));
    array->data_start =
        (jl_gc_mark_data_t *)malloc_s(JL_GC_QUEUE_INIT_SIZE * sizeof(jl_gc_mark_data_t));
    array->size = JL_GC_QUEUE_INIT_SIZE;
