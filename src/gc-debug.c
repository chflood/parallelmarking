...
void gc_time_summary(int sweep_full, uint64_t start, uint64_t end, uint64_t freed,
                     uint64_t live, uint64_t interval, uint64_t pause)
{
    if (sweep_full > 0)
        jl_safe_printf("TS: %" PRIu64 " Major collection: estimate freed = %" PRIu64
                       " live = %" PRIu64 "m new interval = %" PRIu64 "m time = %" PRIu64
                       "ms\n",
                       end - start, freed, live / 1024 / 1024, interval / 1024 / 1024,
                       pause / 1000000);
    else
...
static size_t pool_stats(jl_gc_pool_t *p, size_t *pwaste, size_t *np, size_t *pnold)
{
    ...
    *pnold = nold;
    if (npgs != 0) {
        jl_safe_printf(
            "%4d : %7lld/%7lld objects (%3lld%% old), %5lld pages, %5lld kB, %5lld kB waste\n",
            p->osize, (long long)nused, (long long)(nused + nfree),
            (long long)(nused ? (nold * 100) / nused : 0), (long long)npgs,
            (long long)((nused * p->osize) / 1024), (long long)(*pwaste / 1024));
    }
...
void gc_stats_all_pool(void)
{
    ...
    jl_safe_printf("%lld objects (%lld%% old), %lld kB (%lld%% old) total allocated, "
                   ...
                   (long long)no, (long long)(no ? (nold * 100) / no : 0),
                   (long long)(nb / 1024), (long long)(nb ? (noldbytes * 100) / nb : 0),
                   (long long)tw, (long long)(nb ? (tw * 100) / nb : 0), (long long)tp);
}
...
void gc_stats_big_obj(void)
{
    ...
    jl_safe_printf(
        "%lld kB (%lld%% old) in %lld large objects (%lld%% old)\n",
...
NOINLINE void gc_mark_loop_unwind(jl_ptls_t ptls, int pc_offset)
{
    ...
    if (bottom.pc_offset > bottom.data_offset) {
        jl_safe_printf("Mark stack unwind overflow -- ABORTING !!!\n");
        return;
    }
    int isroot = 1;
    for (int i = top.offset; i < bottom.pc_offset; i++) {
        void *pc =
            jl_atomic_load_relaxed((_Atomic(void *) *)&array->pc_start[i % array->size]);
        const char *prefix = isroot ? "r--" : " `-";
        isroot = 0;
        if (pc == gc_mark_label_addrs[GC_MARK_L_marked_obj]) {
            gc_mark_marked_obj_t *data = gc_repush_markdata(mark_queue);
            ...
            gc_mark_finlist_t *data = gc_repush_markdata(mark_queue);
            ...
            gc_mark_objarray_t *data = gc_repush_markdata(mark_queue);
            ...
            gc_mark_obj8_t *data = gc_repush_markdata(mark_queue);
            ...
            gc_mark_obj16_t *data = gc_repush_markdata(mark_queue);
            jl_datatype_t *vt = (jl_datatype_t *)jl_typeof(data->parent);
            ...
            gc_mark_obj32_t *data = gc_repush_markdata(mark_queue);
            jl_datatype_t *vt = (jl_datatype_t *)jl_typeof(data->parent);
            ...
            gc_mark_stackframe_t *data = gc_repush_markdata(mark_queue);
            ...
            // module_binding
            gc_mark_binding_t *data = gc_repush_markdata(mark_queue);
